...
describe('Grouped Actions Table', function() {
    ...
    var tableData = [
        {
            ...
            duration: 717799,
            groupDate: new Date(1422946800000).getTime(),
            totalCount: 8
        },
        {
            actions: [
            ...
            duration: 6170996,
            groupDate: new Date(1423033200000).getTime(),
            totalCount: 117
        }
    ];
    ...
    describe('getTableRowItem function', function() {
        ...
        it ('should create table row elements.', function() {
            expect(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr').length).toEqual(3);
        });

        it ('should create rows with a hover class and a text-select class.', function() {
            expect(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr')[1].className).toEqual('hover-enabled text-select');
        });

        it ('should create nested table rows for the selected row.', function() {
            TestUtils.Simulate.click(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr')[1]);
            expect(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr').length).toEqual(7);

            TestUtils.Simulate.click(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr')[6]);
            expect(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr').length).toEqual(27);
        });

        it ('should add a sub-action class to nested table rows.', function() {
            TestUtils.Simulate.click(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr')[1]);
            expect(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr')[2].className).toEqual('hover-enabled text-select sub-action');
        });
    ...
    describe('getTableData function', function() {
        it('should set the value to a calculated duration string if the dataProperty is "duration".', function() {
            ...
            var tableDataContents = tableDataElement.props.children[1];
            expect(tableDataContents.props.title).toBeNonEmptyString();
            expect(tableDataContents.props.children).toEqual('12m');
        });
        ...
        it('should display the number of nested rows and a rowsCollapsed icon if the dataProperty is "groupDate".', function() {
            ...
            var beforeElement = tableDataElement.props.children[0];
            var span = beforeElement.props.children[0];
            var icon = beforeElement.props.children[1];
            expect(span).toEqual(4);
            expect(icon.props.className).toEqual('fa fa-chevron-right');
        });
        ...
        it('should display the number of nested rows and a rowsCollapsed icon from props if the dataProperty is "groupDate".', function() {
            ...
            var beforeElement = tableDataElement.props.children[0];
            var icon = beforeElement.props.children[1];
            expect(icon.props.className).toEqual('test-rows-collapsed');
        });
        ...
        it('should display the number of nested rows and a rowsExpanded icon if the dataProperty is "groupDate" and the row was clicked.', function() {
            ...
            var icon = beforeElement.props.children[1];
            expect(span).toEqual(4);
            expect(icon.props.className).toEqual('fa fa-chevron-down');
        });
        ...
        it('should display the number of nested rows and a rowsExpanded icon from props if the dataProperty is "groupDate" and the row was clicked.', function() {
            ...
            var beforeElement = tableDataElement.props.children[0];
            var icon = beforeElement.props.children[1];
            expect(icon.props.className).toEqual('test-rows-expanded');
        });
    ...
    describe('getNestedRowTableData function', function() {
        ...
        it('should set the value to a calculated time string from the action duration if the data property is "groupDate".', function() {
            ...
            var tableDataContents = nestedTableDataElement.props.children;
            var title = tableDataContents.props.title;
            var str = tableDataContents.props.children;
