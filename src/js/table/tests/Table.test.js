...
describe('Table', function() {
    ...
    var tableData = [
        {string: 'aaa', integer: -2, mixedCase: 'Aaa', time: 1417455952, percent: 87, status: Moment().subtract(5, 'minutes').valueOf()},
        {string: 'b', integer: 3, mixedCase: 'B', percent: 42, status: Moment().subtract(14, 'minutes').valueOf()},
        {string: 'a', integer: 0, mixedCase: 'a', time: 1416591981, percent: 37},
        {string: 'aa', integer: 2, mixedCase: 'Aa', time: 1417715098, percent: 96, status: Moment().subtract(3, 'days').valueOf()},
        {string: 'aab', integer: -1, mixedCase: 'aAb', percent: 8, status: Moment().subtract(45, 'minutes').valueOf()},
        {string: 'ab', integer: 1, mixedCase: 'aB', percent: 15, status: Moment().subtract(20, 'hours').valueOf()},
        {string: 'aba', integer: 1, mixedCase: 'aBA', time: 1406479597, percent: 67, status: Moment().subtract(50, 'days').valueOf()}
    ];
    ...
    describe('onDataReceived function', function() {
        ...
        it('should show no results if the data returns with an empty array', function() {
            ...
            expect(TestUtils.findRenderedDOMComponentWithClass(table, 'no-results').innerText).toEqual('No results found.');
        });
    });
    ...
    describe('getTableRowItem function', function() {
        ...
        it('should create table row elements', function() {
            ...
            expect(TestUtils.scryRenderedDOMComponentsWithTag(table, 'tr').length).toEqual(8);
        });
        ...
        it('should have a hover-enabled class if row clicks are defined', function() {
            ...
            expect(tableRowComponent.props.className).toEqual('text-select hover-enabled');
        });
    ...
    describe('getBulkSelectionIcon function', function() {
        it('should display the select all icon if no items in the selected list match items in the filtered data set', function() {
            ...
            expect(table.getBulkSelectionIcon(def.cols[0]).props.className).toEqual('fa fa fa-square-o');
            expect(table.getBulkSelectionIcon(def.cols[0]).props.title).toEqual('Select All');
        ...
        it('should display the select all icon passed in on props', function() {
            ...
            expect(table.getBulkSelectionIcon(def.cols[0]).props.className).toEqual('test-select-all');
        });
        ...
        it('should display the deselect all icon if items in the selected list match items in the filtered data set', function() {
            ...
            expect(table.getBulkSelectionIcon(def.cols[0]).type).toEqual('i');
            expect(table.getBulkSelectionIcon(def.cols[0]).props.className).toEqual('fa fa-minus-square-o');
            expect(table.getBulkSelectionIcon(def.cols[0]).props.title).toEqual('Deselect All');
        });
        ...
        it('should display the deselect all icon passed in on props', function() {
            ...
            expect(table.getBulkSelectionIcon(def.cols[0]).props.className).toEqual('test-deselect-all');
        });
    });
    ...
    describe('getSortIcon', function() {
        it('should append the correct default classNames to the icon', function() {
            ...
            table = TestUtils.renderIntoDocument(<Table {...propsWithCustomIcons} />);
            table.requestData();
